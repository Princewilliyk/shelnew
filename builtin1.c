...
int unset _alias(info_t *info, char *str)
{
	char *p, c;
	int ret;
...
/**
 * _myalias - mimics the alias buitin (man alias)
 * @info: Structure containing potential arguments, usd to maintain
 *	constant function prototype.
 * Return: Always 0
 */

int _my alias(info_t *info)
{
	int i = 0;
	char *p = NULL;
	list_t *node = NULL;
	...
	{
	p = _strchr(info_ > argv[i], '=');
	if (p)
		set_alias(info, info_ > argv[i]);
	else
